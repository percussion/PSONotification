...
public class PSExitNotifyBase implements IPSResultDocumentProcessor
{
   ...
   protected static Log log = LogFactory.getLog(PSExitNotifyBase.class);
   
   private IPSSystemService ssvc = null; 
   
   private IPSTokenSubstitutionProcessorFactory tokenProcessorFactory = null; 
      ...
      throws PSExtensionException
   {
      ...
      if(ssvc == null)
      {
         ssvc = PSSystemServiceLocator.getSystemService(); 
      }
      if(tokenProcessorFactory == null)
      {
         tokenProcessorFactory = new PSTokenSubstitutionProcessorFactory();         
      }
         ...
         throws PSParameterMismatchException, PSExtensionProcessingException
   {
      ...
      try
      {
         ...
         sendNotifications(contentURL, contentID, workflowID, transitionID, fromStateID,
    ...
    */
   @SuppressWarnings("unchecked")
   private void sendNotifications(String contentURL, int contentId, int workflowID, 
         ...
         throws PSEntryNotFoundException, PSMailException, SQLException
   {
      ...
      try
      {
         ...
         {
            fromStateUserList = this.getStateUserList(request, contentId, wfRoleInfo, wfRoleInfo.getFromStateCauc(), workflowID, 
         ...
         {
            toStateUserList = this.getStateUserList(request, contentId, wfRoleInfo, wfRoleInfo.getToStateCauc(), workflowID, 
   ...
   /**
    * Builds a writable message context with the default 
    * host and domain already present. 
    * @return the message context. 
    */
   protected IPSMailMessageWritable buildMessageContext()
   {
      String emsg;
      IPSMailMessageWritable messageContext = new PSMailMessageCtxWritable();        
      String mailDomain =  PSWorkFlowUtils.properties.getProperty("MAIL_DOMAIN", "");
      if(StringUtils.isBlank(mailDomain))
      {
         emsg ="Mail domain is required. Check rxworkflow.properties";
         log.error(emsg);
         throw new IllegalStateException(emsg); 
      }
      messageContext.setMailDomain(mailDomain);
      
      String smtpHost = PSWorkFlowUtils.properties.getProperty("SMTP_HOST", "");
      if(StringUtils.isBlank(smtpHost))
      {
         emsg = "SMTP Host is required. Check rxworkflow.properties"; 
         log.error(emsg);
         throw new IllegalStateException(emsg); 
      }
   ...
   protected void setToAddress(IPSRequestContext request, IPSMailMessageWritable messageCtx, String usersTo, String communityId)
   {
      ...
      List<String> toList = PSWorkFlowUtils.tokenizeString(usersTo,PSWorkFlowUtils.EMAIL_STRING_DELIMITER);
   ...
   protected void setCCAddress(IPSRequestContext request, IPSMailMessageWritable messageCtx, String users, String communityId) 
   {
      ...
      List<String> ccList = PSWorkFlowUtils.tokenizeString(users,PSWorkFlowUtils.EMAIL_STRING_DELIMITER); 
   ...
   /**
    * Sets the URL included in the body of the message. The URL is URLEncoded 
    * with the UTF-8 character set per the W3C recommendatation. 
    * @param request the users request context
    * @param messageCtx the current message
   ...
   protected void setMessageUrl(IPSRequestContext request, IPSMailMessageWritable messageCtx, String messageUrl)
   {
      ...
      try
      {
         url = URLEncoder.encode(messageUrl,"UTF-8");
   ...
   {
      List<String> userEmailAddressList; 
    ...
    */
   @SuppressWarnings("unchecked")
   protected List getStateUserList(IPSRequestContext request, int contentId,
      ...
      throws SQLException
   {
      ...
      try
      {
         ...
         List stateRoleNotificationList = PSWorkflowRoleInfo
               .getStateRoleNameNotificationList(stateRoleContext,contentId);
         ...
         List fromStateAdhocActorNotificationList = PSWorkflowRoleInfo
               .getStateAdhocActorNotificationList(stateAdhocContext,
                     stateRoleContext, contentId, request);
    ...
    */
   @Deprecated
   protected static String parseTokens(String str, Map<String,String> tokenValues)
   {
      if ((str != null) && (str.trim().length() > 0))
      {
         Iterator<Map.Entry<String,String>> it = tokenValues.entrySet().iterator();
         while (it.hasNext())
         {
    ...
    *  
    *
    * @throws             PSMailException if the mail cannot be sent. 
    */
   protected void sendMail(IPSRequestContext request, IPSMailMessageWritable messageContext)
      throws PSMailException
   {
      Validate.notNull(ssvc); 
      ssvc.sendEmail(messageContext);
    }

   protected void processTokens(IPSRequestContext request, int contentId, IPSMailMessageWritable messageContext,
         String subject, String body) throws RepositoryException
   {
      IPSTokenSubstitutionProcessor tkp = tokenProcessorFactory.getTokenSubstitutionProcessor();
      tkp.parseFields(subject);
      tkp.parseFields(body); 
      
      tkp.processTokenValues(contentId, request); 
      
      Map<String, String> extraTokens = addExtraTokens(request, contentId); 
      
      if(extraTokens != null && !extraTokens.isEmpty())
      {
         tkp.addExtraTokens(extraTokens);
      }
      if(dateFormat != null)
      {
         tkp.setDateFormat(dateFormat);
      }
      subject = tkp.replaceTokens(subject);
      log.debug("Message Subject is " + subject); 
      messageContext.setSubject(subject); 
      
      body = tkp.replaceTokens(body); 
      log.debug("Message Body is " + body); 
      messageContext.setBody(body); 
   }
   
   protected Map<String,String> addExtraTokens(IPSRequestContext request, int contentId)
   {
   ...
   protected static final String DATE_FORMAT_KEY = "NOTIFICATION_DATE_FORMAT";
    ...
    */
   protected void setSsvc(IPSSystemService ssvc)
   {
      this.ssvc = ssvc;
   }

   protected IPSTokenSubstitutionProcessorFactory getTokenProcessorFactory()
   {
      return tokenProcessorFactory;
   }

   protected void setTokenProcessorFactory(
         IPSTokenSubstitutionProcessorFactory tokenProcessorFactory)
   {
      this.tokenProcessorFactory = tokenProcessorFactory;
   }
